(function () {
  try {
    ...
    var variation_name = "cre-t-21";
     ...
     var SWIPER_CACHE_KEY = "cre-t21-swiper-js-8.3.2";
    ...
    /* rAF-throttles a handler so scroll/resize work runs at most once per
       animation frame instead of once per event (CR-04). */
    ...
    //inject scripts
    const injectScripts = () => {
      // CR-06: don't re-fetch/re-eval the bundle if Swiper is already loaded.
      if (typeof window.Swiper === "function") return Promise.resolve();

      // CR-05: read a session-cached copy before hitting the network again.
      var cached = null;
      try {
        cached = sessionStorage.getItem(SWIPER_CACHE_KEY);
      } catch (e) {
        cached = null;
      }
      ...
      if (cached) {
        ...
        if (typeof window.Swiper !== "function") {
          throw new Error("Swiper failed to initialize from cached copy");
        }
      ...
      return fetch(SWIPER_JS_URL)
        ...
        .then((code) => {
          ...
          try {
            sessionStorage.setItem(SWIPER_CACHE_KEY, code);
          } catch (e) {
            /* sessionStorage unavailable/full — non-fatal, just skip caching */
          }
        });
    };
...
'</svg></span> Immediate access to your member benefits</p>' +
    ...
    function initSwiper() {
      ...
      var throttledRefresh = rafThrottle(refresh);
      window.addEventListener("load", throttledRefresh);
    ...
    function init() {
      ...
      /* CR-01/CR-02: guard against a second init() run (script
         re-injected, SPA-style re-render, bfcache restore, etc.)
         rebuilding the section and re-wiring every listener on top of
         the first run. */
      if (window.__creT21Inited || mountEl.querySelector(".Cre_explore-membership_redesing")) return;
      ...
      injectScripts()
        ...
        .catch(function (err) {
          /* CR-03: Swiper failed to load (CDN blocked, network blip, etc.) —
             hide the now-inert nav/pagination controls and fall back to the
             plain scrollable layout defined in vB.css for
             .testimonials-swiper:not(.swiper-initialized). */
          if (debug) console.log(err, "Swiper failed to load in Test " + variation_name);
          var nav = document.querySelector(".testimonials-nav");
