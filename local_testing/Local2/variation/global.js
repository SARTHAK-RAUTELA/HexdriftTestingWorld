(function () {
  try {
    ...
    /* -----------------------------------------------------------------
       VWO goal helper
       Used by the "AFP21 - Scroll testimonials" (V1 only) goal, fired on:
         1) pagination bullet click
         2) arrow click that changes the slide
         3) dragging/swiping the slider
       ----------------------------------------------------------------- */
    ...
    function buildValueTableHTML() {
      ...
      valueTableData.forEach(function (group) {
        ...
        group.items.forEach(function (item) {
          rows += `<tr><td data-label="Benefit">${item.benefit}</td><td data-label="AFP Non-Member">${item.nonMember}</td><td data-label="AFP Member" class="is-included"><span class="check">${checkIconSVG("#0076A8")}</span> ${item.member}</td></tr>`;
        });
      });
      return rows;
    }
...
<a href="#join" class="btn btn--primary">JOIN AFP</a>
    ...
    function initSwiper() {
      ...
      /* -----------------------------------------------------------------
         ...
         Fires window.VWO.event("scrollTestimonials") on:
           1) a pagination bullet click
           2) an arrow (prev/next) click
           3) dragging/swiping the slider track
         Each trigger fires independently and is not de-duplicated against
         the others — VWO's own goal-conversion counting handles repeats,
         so this simply reports every qualifying interaction.
         ----------------------------------------------------------------- */
