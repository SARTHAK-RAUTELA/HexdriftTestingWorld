(function () {
  try {
    ...
    function generateAccordianItem() {
      ...
      return `<div class="cre-t-144-accordion_item oxy-pro-accordion_item">
    <button class="cre-t-144-accordion_header" aria-expanded="false">
        <span class="cre-t-144-accordion_title-area oxy-pro-accordion_title-area">
            <h4 class="cre-t-144-accordion_title oxy-pro-accordion_title">${Question}</h4>
            <span class="cre-t-144-accordion_subtitle"></span>
        </span>
        <span class="cre-t-144-accordion_icon cre-t-144-accordion_icon-animate oxy-pro-accordion_icon oxy-pro-accordion_icon-animate">
            <svg class="cre-t-144-accordion_toggle-icon oxy-pro-accordion_toggle-icon">
                <use xlink:href="#Bootstrapbootstrap-icon-plus"></use>
            </svg>
        </span>
    </button>
    <div class="cre-t-144-accordion_body oxy-pro-accordion_body">
        <div class="cre-t-144-accordion_content">
            <p>${Answer}</p>
        </div>
    </div>
</div>`
    ...
    function addVetApprovedLink() {
      waitForElement('.oxy-site-navigation.header-nav ul li.menu-item', function () {
        ...
        if (!document.querySelector('.cre-t-144-vetApprovedLink')) {
          listItem.insertAdjacentHTML('beforebegin', `<li class='cre-t-144-vetApprovedLink'> Vet Approved </li>`)
        }
      })
    }

    function eventHandler() {
      live('.cre-t-144-vetApprovedLink', 'click', function () {
        scrollToEl('.cre-t-144-accordion_header');
        let NewAccordain = document.querySelector('.cre-t-144-accordion_item');
        if (NewAccordain && !NewAccordain.classList.contains('active')) {
          NewAccordain.querySelector('.cre-t-144-accordion_header').click();
        }
      })

      live('.cre-t-144-accordion_item button', 'click', function (e) {
        e.preventDefault();
        e.stopImmediatePropagation()
        var $button = jQuery(this);
        var $item = $button.closest('.cre-t-144-accordion_item');
        ...
        jQuery('.oxy-pro-accordion_item.active').not($item[0]).each(function () {
          ...
          if ($active.hasClass('cre-t-144-accordion_item')) {
    ...
    function init() {
      ...
      if (!document.querySelector('.cre-t-144-accordion_item')) {
    ...
    waitForjQuery(function () {
      waitForElement(".faq-container .oxy-pro-accordion .oxy-pro-accordion_item:first-child", init);
    })
