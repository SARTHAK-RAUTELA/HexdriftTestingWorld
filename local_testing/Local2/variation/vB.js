(function () {
  try {
    /* main variables */
    var debug = 1;
    var variation_name = "cre-t-136";
    var cookie_name = "cre-t-136-cookie";
    ...
    function setBmCookie() {
      document.cookie = cookie_name + "=cre-t-136-variation; path=/";
    }
    ...
    function addElement() {
      ...
      var html = `<div class="cre-t-136-container" style="display: none;">
        <div class="cre-t-136-wrapper">
          <div class="cre-t-136-close-icon">
          ...
          <div class="cre-t-136-content">
            <div class="cre-t-136-header">
              <div class="cre-t-136-hero-title">Looking for ${updateName}?</div>
            </div>
            <div class="cre-t-136-subheader">
              <div class="cre-t-136-subheader-title">We’ve reviewed 23 renters insurance providers to bring you the top 7—many with <span>instant online approval</span> and <span>fast claim payouts</span>. Start with our top picks below (${updateName} didn’t make the list).</div>
            </div>
          </div>
        </div>
      </div> `;

      if (!document.querySelector(".cre-t-136-container")) {
    ...
    function init() {
      ...
      if (getCookie(cookie_name) === "cre-t-136-variation") return;
      ...
      if (!window.cre_t_123_event) {
        ...
        live(".cre-t-136-close-icon", "click", function () {
          setBmCookie();
          document.body.classList.remove(variation_name);

          var container = document.querySelector(".cre-t-136-container");
          if (container) {
            container.remove();
          }
        });
