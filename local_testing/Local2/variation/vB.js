  ...
  try {
    ...
    function reorder(mode) {
      ...
      if (mode === "lowest-price") {
        ...
        // "Best Rated" is the page's own default order, not a re-sort of ours - per the client,
        // that default already reflects their overall ranking (rating is only one input into it),
        // so just replay whatever order captureOrder() saw for the current zip/breed filter.
    ...
    function eventListeners() {
      ...
      // Observing #comparison-section (not .plan-repeater) is load-bearing: the site replaces the
      // .plan-repeater element wholesale on a zip/breed re-render rather than just mutating its
      // children, so an observer bound to that specific node reference goes permanently silent the
      // first time it happens - it's watching a detached element and never rewires itself to the
      // new one. #comparison-section itself is never replaced, so anchoring here means every later
      // re-render (another filter change, "Show More" appending more cards, etc.) keeps being seen,
      // however many times the inner repeater gets swapped out. The extra noise from this also
      // covering the filters row (where our own sort field/copy live) is harmless: refresh()'s
      // isFreshRender check throws those calls away as no-ops, same as it already does for our own
      // rt-sorting/rt-sorted-in class churn on the repeater.
