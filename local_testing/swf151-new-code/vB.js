(function () {
  ...
  try {
    /* Sort field UI + "Lowest Price" ordering + the pinned card mirroring whichever listing is #1.
       Best Rated re-ranks nothing, so that state is the control. Everything injected is `cre-t-151-`. */
    ...
    var DEFAULT_SORT_MODE = "best-rated"; // what the pill should read on first load
    ...
    // The site's rating bar tracks DOM order, which `order` cannot move - vB.css section 0b re-points
    ...
    /* Hands the rating bar to whichever card is in visual slot 1 (vB.css section 0b). Runs in BOTH
