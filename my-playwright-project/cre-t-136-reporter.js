/**
 * CRE-T-136 Custom HTML Reporter
 * Generates cre-t-136-qa-report.html for Renters Insurance Gurus — Insurer Alert Box.
 * Filters on spec files containing "cre-t-136".
 */
...
const FIGMA_IMG = 'C:\\Users\\Sarthak Rautela\\Downloads\\Group 2.png';
const OUTPUT    = path.join(__dirname, '../local_testing/Local2/cre-t-136-qa-report.html');
...
class CreT136Reporter {
  constructor() {
    this._results = [];
  }

  onTestEnd(test, result) {
    if (!test.location.file.includes('cre-t-136')) return;
  ...
  _generate() {
    ...
    const statefarmShots  = allScreenshots('tc14-statefarm');
    const comparisonShots = allScreenshots('tc18-comparison-page');
...
<title>CRE-T-136 QA Report — Renters Insurance Gurus — Insurer Alert Box</title>
<style>
:root {
...
<!-- COVER -->
<div class="cover">
  <div class="cover-brand">Brillmark QA · Renters Insurance Gurus A/B Testing</div>
  <h1>CRE-T-136 QA Report — <span>Insurer Alert Box</span></h1>
  <div class="sub">Automated QA for the CRE-T-136 variation on Renters Insurance Gurus — a dismissible alert box injected above comparison results when the <code>&amp;insurer=</code> query parameter is present. Same methodology as CRE-T-123 (Pet Insurance Gurus), ported to the Renters Insurance Gurus site. Dynamic company name insertion tested across multiple insurer values and URL encodings.</div>
  <div class="meta">
    <span>📅 ${dateStr}</span>
    <span>🌐 rentersinsurancegurus.com — Homepage + /comparison/</span>
  ...
  <div class="badge-row">
    ...
    <span class="badge-pill teal">🏠 Renters Insurance Gurus</span>
...
<div class="toc">
  ...
  <ol>
    ...
    <li><a href="#discrepancies">Figma vs Code Discrepancies (Bugs Found)</a></li>
...
<div class="info-card">
  <table>
    <tr><td>Test ID</td><td><strong>CRE-T-136</strong></td></tr>
    <tr><td>Variation Class</td><td><code>cre-t-136</code></td></tr>
    <tr><td>Test Name</td><td>Insurer Alert Box (same methodology as CRE-T-123 / SWF123, ported to Renters Insurance Gurus)</td></tr>
    <tr><td>Test Type</td><td>Convert.com A/B Test — Dynamic Insurer Alert</td></tr>
    <tr><td>Client</td><td>Renters Insurance Gurus</td></tr>
    <tr><td>Audience Targeting</td><td><strong>Only users with <code>&amp;insurer=</code> query parameter in URL</strong> (enforced by Convert.com audience segment — not by JS)</td></tr>
    ...
    <tr><td>Dismiss Behavior</td><td>Click <code>.cre-t-136-close-icon</code> → removes element, sets cookie, removes body class</td></tr>
    <tr><td>Cookie Guard</td><td><code>cre-t-136-cookie=cre-t-136-variation</code> — prevents re-show after dismiss</td></tr>
...
<table class="url-table">
  ...
  <tbody>
    <tr><td><strong>Control</strong></td><td><code>https://rentersinsurancegurus.com/?cro_mode=qa&amp;_conv_eforce=100052370.1000256205&amp;insurer=MetLife+Life+Insurance</code></td><td>1000256205</td></tr>
    <tr><td><strong>Variation — /</strong></td><td><code>https://rentersinsurancegurus.com/?cro_mode=qa&amp;_conv_eforce=100052370.1000256206&amp;insurer=MetLife+Life+Insurance</code></td><td>1000256206</td></tr>
    <tr><td><strong>Variation — /comparison/</strong></td><td><code>https://rentersinsurancegurus.com/comparison/?cro_mode=qa&amp;_conv_eforce=100052370.1000256206&amp;insurer=MetLife+Life+Insurance</code></td><td>1000256206</td></tr>
  </tbody>
</table>

<!-- FIGMA -->
<h2 id="figma">Figma Design Reference</h2>
<div class="section-note">The Figma wireframe (Group 2.png) shows Control (left) vs Variation (right). The variation injects a yellow-background dismissible alert box between the stats bar and the first comparison result. The company name from <code>&amp;insurer=</code> is inserted dynamically in both the title and body text. Per client direction, the placeholder "XX" providers count was finalized as <strong>23</strong>.</div>
${figmaB64
  ? `<div class="ss-wrap"><img src="${figmaB64}" alt="CRE-T-136 Figma Reference"/><div class="ss-caption">CRE-T-136 Figma Design (Group 2.png) — Left: Control | Right: Variation (insurer alert box below stats bar)</div></div>`
  : '<p><em>Group 2.png not found — Figma screenshot skipped.</em></p>'}

<!-- DISCREPANCIES -->
<h2 id="discrepancies">⚠️ Figma vs Code Discrepancies (Bugs Found)</h2>
<div class="alert-warn">⚠️ This variation was cloned from CRE-T-123 (Pet Insurance Gurus). Several leftover artifacts from the source test were found in the ported code. These require developer review before the test goes live.</div>
<div class="diff-card discrepancy" style="margin:20px 0">
  <h3>BUG-01 [HIGH] — Wrong industry noun in body copy</h3>
  <ul>
    <li>Figma says: <code>"We've reviewed <strong>23 renters insurance providers</strong>…"</code></li>
    <li>Code (vB.js) says: <code>"We've reviewed <strong>23 pet insurance providers</strong>…"</code></li>
    <li>Confirmed live on <code>rentersinsurancegurus.com</code> — the alert literally references "pet insurance" on a renters insurance site. Leftover copy from the CRE-T-123 clone source. <strong>Must be fixed before launch.</strong></li>
  </ul>
</div>
<div class="diff-card discrepancy" style="margin:20px 0">
  <h3>BUG-02 [LOW] — Stale default fallback insurer name</h3>
  <ul>
    <li><code>vB.js</code>: <code>var updateName = getQueryVariable("insurer") || "Colonial Penn";</code></li>
    <li>"Colonial Penn" is a life/pet insurance carrier, leftover from the CRE-T-123 source. In practice this is dead code — Convert.com audience targeting only fires the variation when <code>&amp;insurer=</code> is present — but should be updated to a renters-insurance-relevant name for code hygiene.</li>
  </ul>
</div>
<div class="diff-card discrepancy" style="margin:20px 0">
  <h3>BUG-03 [LOW] — Copy-paste event-guard variable name</h3>
  <ul>
    <li><code>vB.js</code> line ~107: <code>if (!window.cre_t_123_event) { window.cre_t_123_event = true; ... }</code></li>
    <li>Should reference <code>window.cre_t_136_event</code>. Harmless in isolation, but if CRE-T-123's own script ever loads on the same page, the shared flag name could cause one test's dismiss-handler binding to be skipped.</li>
  </ul>
</div>
<div class="diff-card discrepancy" style="margin:20px 0">
  <h3>BUG-04 [LOW] — Duplicate CSS rule</h3>
  <ul>
    <li><code>vB.css</code>: <code>.cre-t-136-subheader-title span { font-weight: 600; }</code> is declared twice (once near the base rules, once again lower in the file). Non-blocking — same value both times — but redundant and should be de-duplicated.</li>
  </ul>
</div>
<p style="margin-top:12px;font-size:13px;color:var(--muted)"><strong>Note:</strong> Tests in this suite assert the code's <em>actual</em> current behavior (e.g. "23 pet insurance providers") so failures reflect regressions, not this already-known copy bug. BUG-01 is called out here and must be fixed in <code>vB.js</code> by the developer.</p>
...
<div class="diff-grid">
  <div class="diff-card control">
    <h3>Control</h3>
    <ul>
      <li>Standard Renters Insurance Gurus comparison page</li>
      <li>No alert box above comparison results</li>
      <li>No <code>.cre-t-136-container</code> in DOM</li>
      <li>No <code>body.cre-t-136</code> class</li>
      <li><code>.page-description ul</code> visible (not hidden)</li>
    </ul>
  </div>
  <div class="diff-card variation">
    <h3>Variation (CRE-T-136)</h3>
    <ul>
      ...
      <li>✅ Dynamic title: <strong>"Looking for [Insurer]?"</strong> (font-weight 600, 18px) — matches Figma exactly</li>
      <li>⚠️ Dynamic body: static text + <strong>"[Insurer] didn't make the list"</strong> — static "pet insurance" text is BUG-01</li>
      <li>✅ Bold spans: "instant online approval" + "fast claim payouts"</li>
      <li>✅ X dismiss button (top-right): removes alert + sets cookie</li>
      <li>✅ Cookie guard: <code>cre-t-136-cookie=cre-t-136-variation</code></li>
      <li>✅ <code>body.cre-t-136</code> class added</li>
      <li>✅ <code>.page-description ul</code> hidden (<code>display:none</code>) where present</li>
      ...
      <li>✅ Works identically on <code>/</code> and <code>/comparison/</code></li>
    </ul>
  </div>
</div>
...
${failed === 0
  ? `<div class="alert-pass">✅ All ${passed} test runs passed across all browsers and viewports. The CRE-T-136 variation is functioning correctly — dynamic insurer name injection, dismiss behavior, cookie guard, URL encoding (+ and %20), alternate insurer names, /comparison/ page, and responsive layouts all verified. See the Discrepancies section above for BUG-01 (copy bug) which must still be fixed by the developer.</div>`
...
<ul style="font-size:14px;margin:10px 0">
  ...
  <li><strong>TC-03</strong> — Dynamic title: "Looking for MetLife Life Insurance?"</li>
  <li><strong>TC-04</strong> — Dynamic body: insurer name in "didn't make the list"</li>
  <li><strong>TC-05</strong> — Static body: "We've reviewed 23 pet insurance providers" (BUG-01 vs Figma "renters")</li>
  <li><strong>TC-06</strong> — Bold spans: "instant online approval" + "fast claim payouts" are font-weight 600</li>
  <li><strong>TC-07</strong> — Body class: <code>body.cre-t-136</code> present in variation</li>
  <li><strong>TC-08</strong> — Dismiss button: <code>.cre-t-136-close-icon</code> visible</li>
  <li><strong>TC-09</strong> — Dismiss: clicking X removes container from DOM</li>
  <li><strong>TC-10</strong> — Cookie: <code>cre-t-136-cookie=cre-t-136-variation</code> set after dismiss</li>
  ...
  <li><strong>TC-14</strong> — Different insurer: "State+Farm+Renters+Insurance" shows correct name</li>
  ...
  <li><strong>TC-18</strong> — /comparison/ page: alert present with insurer param</li>
...
<h3>TC-14 — Different Insurer: State Farm Renters Insurance</h3>
${ssSection(statefarmShots, 'State Farm Insurer')}

<h3>TC-18 — /comparison/ Page with Alert</h3>
${ssSection(comparisonShots, '/comparison/ Page')}
...
<ul style="font-size:14px;margin:10px 0">
  ...
  <li><strong>Alert presence (TC-02, TC-18)</strong>: Used <code>page.waitForSelector('.cre-t-136-container')</code> with 40s timeout to handle async injection.</li>
  <li><strong>Dynamic content (TC-03–05)</strong>: Read <code>innerText()</code> from <code>.cre-t-136-hero-title</code> and <code>.cre-t-136-subheader-title</code> and verified insurer name and static copy.</li>
  ...
  <li><strong>Dismiss (TC-09)</strong>: Clicked <code>.cre-t-136-close-icon</code>, waited 600ms, then asserted container count is 0 (element removed from DOM).</li>
  <li><strong>Cookie (TC-10)</strong>: After dismiss, read <code>page.context().cookies()</code> and verified <code>cre-t-136-cookie=cre-t-136-variation</code>.</li>
  ...
  <li><strong>Different insurer (TC-14)</strong>: Substituted <code>insurer=State+Farm+Renters+Insurance</code> in URL and verified correct name in title and body.</li>
  ...
  <li><strong>CSS (TC-16)</strong>: Read <code>getComputedStyle().backgroundColor</code> from <code>.cre-t-136-wrapper</code>.</li>
...
<div class="footer">
  CRE-T-136 QA Report · ${dateStr} · Playwright Automated Tests · rentersinsurancegurus.com<br>
  QA by sarthak@brillmark.com · Spec: cre-t-136-insurer-alert.spec.js · ${allTCs.length} TCs × ${allBrowsers.length} browsers = ${totalRuns} total runs
</div>
