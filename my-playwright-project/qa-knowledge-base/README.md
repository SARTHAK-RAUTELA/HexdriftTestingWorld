...
| [WinkBeds — winkbeds.com](winkbeds/_client-notes.md) | Convert.com | [cre-t-253 Buy Box sub-total line](winkbeds/cre-t-253-buybox-subtotal.md) *(needs `navigator.webdriver` override to run Playwright — see client notes; 60/60 passing across 6 browsers; BUG-01 addon price not reflected in badge)* · [WIN266 Shop Page Select Sub-Total V2](winkbeds/win266-shop-subtotal-v2.md) *(cre-t-266 replaces the shipped cre-t-253 line; 58/119 passed across 7 browsers; BUG-02 HIGH — subtotal line text never updates when the Platform Frame accessory is added, 100% reproducible on 6/7 browser projects, both arms; BUG-01-style Saving-race sampled clean this run but still open per recon; WebKit trio (Safari/Mobile Safari/Tablet) showed slower async-injection timing + a Mobile Safari nav timeout)* |
...
| Price display / multi-day pricing | [G](_shared/test-type-checklists.md) | [SEA316](seaworld/sea316-price-display.md) · [WinkBeds cre-t-253](winkbeds/cre-t-253-buybox-subtotal.md) (running sub-total; site needs `navigator.webdriver` override for Playwright) · [WinkBeds WIN266](winkbeds/win266-shop-subtotal-v2.md) (cre-t-253's successor line; accessory-add live-recompute regressed — confirmed cross-browser, not a flake) |
